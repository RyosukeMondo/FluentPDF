...
public partial class MergeViewModel : ObservableObject
{
    ...
    {
        _logger.LogInformation("AddFiles command invoked");
    ...
    {
        _logger.LogInformation("AddFolder command invoked");
        ...
        try
        {
            ...
            if (folder == null)
            {
                _logger.LogInformation("Folder picker cancelled");
                return;
            }

            var files = await folder.GetFilesAsync();
            var pdfFiles = files.Where(f => f.FileType.Equals(".pdf", StringComparison.OrdinalIgnoreCase));
    ...
    private async Task MergeAsync()
    {
        ...
        try
        {
            ...
            if (outputFile == null)
            {
                _logger.LogInformation("Save picker cancelled");
                return;
            }

            // TODO: Implement merge logic via service
            // var result = await _documentService.MergeDocumentsAsync(Files.Select(f => f.FilePath).ToList(), outputFile.Path);

            DialogApplied = true;
            _logger.LogInformation("Merge completed successfully: {OutputPath}", outputFile.Path);
        }
        catch (Exception ex)
        {
    ...
    private async Task AddFileAsync(StorageFile file)
    {
        ...
        {
            _logger.LogError(ex, "Failed to add file: {FileName}", file.Name);
        }
    }
