...
public partial class ThumbnailsViewModel : ObservableObject, IDisposable
{
    ...
    private async Task LoadThumbnailAsync(ThumbnailItem item)
    {
        ...
        try
        {
            ...
            if (result.IsSuccess && result.Value != null)
            {
                ...
                // Create Avalonia Bitmap on UI thread
                await Dispatcher.UIThread.InvokeAsync(() =>
                {
                    try
                    {
                        ...
                        using (var lockedFrameBuffer = bitmap.Lock())
                        {
                            unsafe
                            {
                                var dest = (byte*)lockedFrameBuffer.Address;
                                fixed (byte* src = pixelData)
                                {
                                    Buffer.MemoryCopy(src, dest, pixelData.Length, pixelData.Length);
                                }
                            }
                        }
                    ...
                    {
                        _logger.LogError(ex, "Error creating Bitmap on UI thread for page {PageNumber}", item.PageNumber);
                        item.IsLoading = false;
                    }
                });
            }
            ...
            {
                await Dispatcher.UIThread.InvokeAsync(() => item.IsLoading = false);
    ...
    private async Task DeletePagesAsync()
    {
        ...
        // TODO: Implement Avalonia confirmation dialog
        // For now, assume confirmed
        var confirmed = true;
        // var confirmed = await DeletePagesDialog.ShowAsync(selectedCount, _document.PageCount);
