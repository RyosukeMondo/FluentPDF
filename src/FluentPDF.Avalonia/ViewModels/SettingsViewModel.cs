...
public partial class SettingsViewModel : ObservableObject, IDisposable
{
        ...
        Services.IThemeService themeService)
    {
        ...
        _themeSubscription = _themeService.ObserveThemeChanges()
            .Subscribe(themeVariant =>
            {
                _logger.LogInformation("Theme changed externally to: {Theme}", themeVariant);
            });

        // Load app settings
        LoadSettings();

        _logger.LogInformation("SettingsViewModel initialized with quality: {Quality}, theme: {Theme}",
            SelectedQualityOption?.Quality, _themeService.CurrentTheme);
    }
